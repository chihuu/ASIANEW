...
export default class ContentPlaylist extends Component {
  ...
  _interact() {
    ...
    if (
      this.props.data.list.listContentPlaylist &&
      this.props.data.list.listContentPlaylist.playlists
    ) {
  ...
  _renderRow(item, sectionID, rowID) {
    ...
    if (
      data.list &&
      data.list.listContentPlaylist &&
      data.list.listContentPlaylist.playlists &&
      data.list.listContentPlaylist.playlists.contents
    ) {
      dataContent = data.list.listContentPlaylist.playlists.contents;
    } else if (
      data.listContentPlaylist &&
      data.listContentPlaylist.playlists &&
      data.listContentPlaylist.playlists.contents
    ) {
    ...
    return (
      <RowContentPlaylist
        ...
        playlistId={idChild}
        _setModalPlaylistVisible={this._setModalPlaylistVisible}
        modalPlaylistVisible={modalPlaylistVisible}
  ...
  _updatePlaylist = items => {
    ...
    const { playlist_id, mode } =
      (data.list.listContentPlaylist &&
        data.list.listContentPlaylist.playlists) ||
      (data.listContentPlaylist && data.listContentPlaylist.playlists);
  ...
  processWhenEnd = () => {
    ...
    const { contents } =
      this.props.data.list.listContentPlaylist.playlists ||
      this.props.data.listContentPlaylist.playlists;

    let rowCurrentItemPlay = 0;

    if (
      parseInt(currentItemPlay) + 1 == contents.length ||
      contents.length <= 1
    ) {
      return false;
    }
  ...
  _setModalPlaylistCreateVisible = visible => {
    this.setState({
      modalPlaylistCreateVisible: visible,
      modalPlaylistVisible: false
    });
  };
  ...
  render() {
    ...
    if (
      list &&
      list.listContentPlaylist &&
      list.listContentPlaylist.playlists &&
      list.listContentPlaylist.playlists.contents
    ) {
        ...
        {loading && data && dataContent && dataContent.contents
          ? <View style={{ flex: 1 }}>
              <View style={[styles.row, styles.paddingDefault]}>
                <Image
                  style={[
                    styles.imageContentPlaylist,
                    styles.imageBorderRadius
                  ]}
                  source={{ uri: dataContent.image }}
                />
                <View style={{ justifyContent: "flex-end" }}>
                  <Text style={styles.sectionContentPlaylistTitle}>
                    {dataContent.name}
                  </Text>
                  <Text style={styles.sectionPlaylistSubTitle}>
                    {dataContent.content_count} songs
                  </Text>
                </View>
              </View>
              ...
              >
                ...
                <View style={[styles.row]}>
                  <TouchableHighlight
                    ...
                    underlayColor={"#ececec"}
                    onPress={() =>
                      this._setModalDeleteConfirmVisible(
                        !this.state.modalDeleteConfirmVisible
                      )}
                  >
                    <View style={styles.row}>
                      <Icon
              ...
              <ModalCreatePlaylist
                ...
                contentId={dataContent.playlist_id}
              ...
              <ModalPlaylist
                ...
                mode={dataContent.mode}
                onPress={() =>
                  this._setModalPlaylistVisible(
                    !this.state.modalPlaylistVisible
                  )}
              >
                <TouchableWithoutFeedback
                  onPress={() =>
                ...
                >
                  ...
                  <ListAll
                    style={{ height: "30%" }}
                    userInfo={this.props.userInfo}
                    playlistId={dataContent.playlistId}
              ...
              <ModalDeleteConfirm
                modalVisible={this.state.modalDeleteConfirmVisible}
                onCancel={() =>
                  this._setModalDeleteConfirmVisible(
                    !this.state.modalDeleteConfirmVisible
                  )}
                onDelete={this._deletePlaylist}
                playlistId={dataContent.playlist_id}
              />
